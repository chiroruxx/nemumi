...
- **決めたこと**:
    ...
    - 初回公開はトップページだけで完結させる。
    - セクションはプロフィール、記事・活動、登壇・スライド、コミュニティの順に並べる。
    - Articles と Activities
      は別々のコレクションで管理し、トップページでは「記事・活動」の一覧にまとめて表示する（[ADR 0009](../adr/0009-content-collections.md)）。
    ...
    - Articles と Activities は「記事・活動」の一覧にまとめ、日付の新しいものを上に表示する。
    - 一覧では記事に掲載サービス名、活動に活動種別を表示する。
    - 記事には公開日、活動には開始日を表示する。
    ...
    - Activities の複数日開催は開始日と終了日を表示する。
