...
- Talks / Slides（登壇・発表履歴）
    ...
    - 一覧ではイベント名・開催日とスライドへのリンクを表示し、サムネイルは使わない。
- Communities（コミュニティ）
    - 主催している勉強会やコミュニティの紹介。関連ページへのリンクも各項目に含める。
- Activities（活動履歴）
    - コミュニティ運営やイベントなどの活動実績。関連ページへのリンクも各項目に含める。

### MVP（最初の公開範囲）

- 「載せたいコンテンツのイメージ」に挙げた Profile、Articles / Blog、Talks / Slides、Communities、Activities を含める。
...
- **決めたこと**:
    ...
    - プロフィールに短い自己紹介文を載せる。
    - プロフィールに GitHub と X のアカウントリンクを載せる。
    - 活動拠点は独立した欄にせず、自己紹介文に含める。
    - 得意な領域は独立した欄にせず、自己紹介文に含める。
    - 経歴の年表や独立した欄は設けない。
    - 現在の仕事や役割はプロフィールに載せない。
    - 対応言語は日本語のみとする。
    - MVP には「載せたいコンテンツのイメージ」に挙げた全項目を含める。
    - 初回公開はトップページだけで完結させる。
    - セクションはプロフィール、記事、登壇・スライド、コミュニティ、活動履歴の順に並べる。
    - Communities と Activities は別々のセクションにする（[ADR 0009](../adr/0009-content-collections.md)）。
    ...
    - Articles は公開日の新しいものを上に表示する。
    - Articles の一覧に公開日を表示する。
    - Articles の一覧に短い説明文は載せない。
    - Articles
      の一覧に掲載サービス名を表示する。現状は、はてなブログと「しずかな」を想定する。
    - 掲載サービスは固定リストにせず、必要になったら追加方法を検討する。
    - Talks / Slides は開催日の新しいものを上に表示する。
    - Talks / Slides の一覧に開催日を表示する。
    - Talks / Slides の一覧にサムネイルは使わない。
    - Activities
      は開始日の新しいものを上に表示し、複数日開催は開始日と終了日を表示する。
...
- **やること**:
    - 短い自己紹介文の執筆
    - アイコン画像など、必要な画像を用意する。写真を使う場合は EXIF の位置情報を削除する。
    - 登壇履歴、コミュニティ活動のデータ化
